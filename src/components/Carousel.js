...
export default function Carousel({ children, refContainer }) {
  ...
  const isAllowToShift = useRef(true);
  const cards = useRef();
  ...
  const onClick = useCallback((e) => {
    e = e || window.event;
    !isAllowToShift.current && e.preventDefault();
  }, []);
  ...
  return (
    <div
      ref={refDragHandler}
      className="flex -ml-4 md:-ml-[7px] flex-row relative"
      style={{ paddingLeft: containerClientRect?.left - 20 || 0 }}
    >
      {children}
    </div>
  );
